...
Установщик автоматически ищет `py.exe`, затем `python.exe`. Для корпоративной или переносимой установки задайте интерпретатор явно:
...
Для индикаторов и управления локальной LLM запустите во втором окне PowerShell монитор:

```powershell
$env:PYTHONPATH = "$PWD\sto_rag"
$env:NORMCONTROL_LLAMA_EXE = 'C:\LM\llama\llama-server.exe'
$env:NORMCONTROL_LLAMA_LAUNCHER = 'C:\LM\llama\start_llama.bat'
.\.venv\Scripts\python.exe .\sto_rag\llm_sidecar.py .\worker.env
```

Путь к модели и скрипту запуска укажите по вашей установке. Монитор снимает показатели GPU раз в минуту, читает тайминги уже завершённых запросов и не отправляет тестовые запросы в модель. Кнопки управления доступны только администраторам; при остановке и перезапуске активный пакет ждёт сохранения текущей задачи и остаётся доступен для продолжения. Автоперезапуск включается после подтверждённого снижения *обеих* скоростей на 15% для сопоставимых рабочих запросов и не чаще одного раза в час. На других компьютерах без `nvidia-smi` графики GPU будут показывать отсутствие данных.
